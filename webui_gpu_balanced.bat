@echo off
REM Balanced speed/memory (4-8GB VRAM)
echo Starting WebUI with GPU acceleration - Balanced speed/memory (4-8GB VRAM)...
echo.
...
set COMMANDLINE_ARGS=--skip-python-version-check --skip-install --precision autocast --opt-split-attention --opt-sub-quad-attention --medvram

REM Use existing PyTorch installation
set TORCH_COMMAND=echo "Using existing PyTorch installation"

echo GPU Configuration: Balanced speed/memory (4-8GB VRAM)
echo.
