@echo off
REM RTX 3080 Laptop GPU Conservative Configuration
REM Optimized for 16GB VRAM - Conservative Mode (4-8GB usage)
echo Starting WebUI with RTX 3080 Conservative Mode...
echo Target: 25-40 seconds per 512x512 image, maximum stability
echo.
...
REM RTX 3080 Conservative GPU Environment Variables
set PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:256,roundup_power2_divisions:4
...
REM RTX 3080 Conservative Performance Flags (WebUI v1.10.1 compatible)
...
REM Maximum stability flags
set COMMANDLINE_ARGS=%COMMANDLINE_ARGS% --disable-nan-check

echo RTX 3080 Conservative Configuration:
echo - GPU: NVIDIA GeForce RTX 3080 Laptop GPU (16GB VRAM)
echo - Mode: Conservative (25-40s per 512x512 image)
echo - VRAM Usage: 4-8GB (maximum stability)
echo - Python 3.13 compatibility: PRESERVED
echo - All previous fixes: ACTIVE
echo - Memory management: Very conservative
echo - Precision: Mixed (autocast) with maximum stability
echo - Ideal for: Testing, debugging, or thermal management
echo.
