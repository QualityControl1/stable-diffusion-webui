@echo off
REM Low VRAM mode (2-4GB VRAM)
echo Starting WebUI with GPU acceleration - Low VRAM mode (2-4GB VRAM)...
echo.
...
set COMMANDLINE_ARGS=--skip-python-version-check --skip-install --precision autocast --lowvram --opt-split-attention --opt-sub-quad-attention

REM Use existing PyTorch installation
set TORCH_COMMAND=echo "Using existing PyTorch installation"

echo GPU Configuration: Low VRAM mode (2-4GB VRAM)
echo.
